...
Обычно effect запускается сразу после создания — чтобы прочитать сигналы и построить зависимости. Но если тебе нужно сначала подготовить окружение, используй `{ lazy: true }`(**по default стоит true**): эффект не выполнится мгновенно, а начнёт жить только когда что-то реально изменится.
