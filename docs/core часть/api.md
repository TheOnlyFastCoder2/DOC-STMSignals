...
По умолчанию `{ lazy: true }`: эффект **не выполняется сразу при создании**. Он “встаёт на дежурство” и впервые запустится только тогда, когда что-то действительно изменится и его разбудит планировщик. Это удобно, когда ты не хочешь лишний стартовый прогон (например, при маунте/инициализации), а хочешь реакцию только на реальные изменения.

```ts live noInline
const s = signal(0);

effect(() => {
  console.log('effect:', s.v);
}); // lazy: true по умолчанию

s.v = 1; // теперь эффект впервые выполнится
```

Если наоборот нужно “выполниться прямо сейчас, чтобы сразу построить зависимости”, явно отключай ленивость:
