...
Ок — тогда перепишем так, будто **по умолчанию `lazy: false`**, а “ловушка” появляется только когда ты **явно включаешь** `{ lazy: true }`.

---

### Signal.u — “вторая дверь”: читать и писать тихо

И вот тут начинается первая “вау-фишка”, о которой любят забывать в доках, а потом страдать в проде: у Signal есть вторая дверь — `.u`. Это режим “я просто посмотрю” и “я тихонько поправлю”: `signal.u` читает значение без подписки, а `signal.u = ...` записывает его без уведомления подписчиков. Представь сервисный вход в здание: через парадный (`.v`) заходят все и включается свет (строятся зависимости, будятся эффекты), а через чёрный (`.u`) можно занести коробку ночью и никого не будить.


##### Обычный режим (lazy по умолчанию выключен): `.u` никого не будит

```ts live noInline
const s = signal(0);

effect(() => console.log('watch', s.v)); // lazy:false по умолчанию

s.u = 123;               // тихо: эффект НЕ проснётся
console.log('u =', s.u); // тоже без подписки
```

Здесь всё честно и предсказуемо: эффект уже подписался (он стартанул сразу, прочитал `s.v`), но `.u` — это “без уведомлений”, поэтому `watch` не будет.

---

#####  Ловушка: `{ lazy: true }` + `.u` в одном тике может выглядеть как “реакция”

Если ты **включил** lazy вручную, первый запуск эффекта откладывается. И пока он не стартовал, он ещё не подписался. Поэтому запись в `.u` по-прежнему “тихая”… но когда отложенный первый запуск всё-таки случится, эффект просто прочитает уже новое значение — и создаётся иллюзия, что он “отреагировал”.

```ts live noInline
const s = signal(0);

effect(() => console.log('watch', s.v), 'low', { lazy: true });

s.u = 123;
console.log('u =', s.u);

// чуть позже, когда scheduler впервые запустит effect:
// watch 123
```

Ключевая мысль простая: **`.u` ничего не триггерит**. Просто lazy-effect впервые стартует позже и видит актуальное значение.

---

#####  Как не путаться

Если тебе важно, чтобы эффект **подписался сразу** и “увидел старт”, оставляй default (`lazy:false`) или явно ставь:

```ts
effect(fn, 'normal', { lazy: false });
```

А `.u` используй как инструмент “поставить значение тихо”, когда ты действительно не хочешь будить реактивность (инициализация, гидрация, техническая синхронизация).
...
if ('__stmError' in snap) {
  ...
  console.log('ERR =', snap.__stmError?.message)
} else {
...
### Lazy-эффекты — когда эффект не стартует сам по себе

Обычный `effect(...)` стартует сразу: один раз выполняет коллбек, читает нужные `.v`, строит зависимости — и дальше уже честно реагирует на изменения. Но иногда этот первый прогон просто лишний: ты ещё “собираешь сцену” при маунте/инициализации и не хочешь, чтобы эффект успел выстрелить в середине подготовки. Тогда помогает `{ lazy: true }`: эффект создаётся сразу, но **первый запуск откладывается на планировщик** — он включится чуть позже и прочитает уже актуальные значения, после чего станет обычным участником графа.

:::warning

##### React StrictMode (dev) + lazy: true: не пугайся “странных” логов

В **React StrictMode** (только в dev) компоненты и эффекты могут инициализироваться дважды (`mount → unmount → mount`). Из-за этого `effect(..., { lazy: true })` иногда выглядит так, будто он “среагировал” на запись через `.u`, хотя `.u` никого не уведомляет.

###### Почему так происходит
В **lazy-режиме** первый запуск эффекта может случиться чуть позже, и если до этого ты успел сделать `s.u = 123`, то эффект просто впервые стартует и читает уже новое значение (`123`). В StrictMode это ещё и повторяется во втором прогоне, поэтому порядок логов может быть совсем “рваным”.


###### Пример, который может удивить в StrictMode:
...
В **обычном режиме** ты чаще увидишь:
```
watch 0
u = 123
```

А в **StrictMode (dev)** может прилететь что-то вроде:
```
u = 123
watch 123
watch 0
...
1. **`.u` не триггерит подписчиков** — эффект не “отреагировал на `.u`”, он просто поздно впервые запустился и прочитал актуальное значение.
2. Это **dev-особенность StrictMode**. В production порядок обычно стабильнее.
3. Если тебе нужен предсказуемый “первый прогон” (сразу подписаться и снять стартовый снимок) — используй `{ lazy: false }`.
:::
...
effect(() => {
  console.log('run immediately:', s.v);
}); // lazy: false по умолчанию
```
...
}, 'normal', { lazy: true }); // не стартует сразу

s.v = 1; // теперь эффект впервые выполнится
```

`lazyEffect` — это обычная обёртка над `effect/Effect`, которая просто создаёт эффект с включённым флагом `lazy: true` (и при необходимости прокидывает остальные опции дальше).

```ts live noInline
const s = signal(0);

lazyEffect(() => {
  console.log('lazy:', s.v);
});
...
lazyEffect(() => console.log('HIGH', s.v), 'high');
lazyEffect(() => console.log('LOW', s.v), 'low');
lazyEffect(() => console.log('SYNC', s.v), 'sync');
...
// setSchedulerMode('frame');
const s = signal(0);
let seen = 0;

effect(() => {
  console.log('[effect run] s =', s.v);
  seen = s.v;
});

console.log('[before set] seen =', seen);
console.log('--- set to 42 ---');
s.v = 42;
console.log('[after set] seen =', seen);
```
...
effect(() => console.log('work:', s.v), 'low');
const e = effect(() => console.log('to work high:', s.v), 'low');
...
effect(() => console.log('frame', s.v), 'low'); // <-- запуск сразу => увидит 0

console.log('--- set to 1 ---');
s.v = 1;      // попало в очередь кадра
console.log('--- before flush ---');
flushSync();  // принудительно дреним очередь сейчас
