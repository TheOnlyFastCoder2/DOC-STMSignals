...
`untracked(fn)` используется **внутри effect/computed**, чтобы то, что происходит в колбэке,  
**не добавляло новых зависимостей** для этого эффекта.
...
effect(() => {
  ...
  // Временно выходим из реактивного контекста
  untracked(() => {
    // Здесь можно читать и писать сигналы,
    // но effect не будет зависеть от b
    b.v = a.v * 2;
    console.log('debug b =', b.v);
  });
});

// ↓ это НЕ вызовет effect, потому что b не в его зависимостях
b.v = 32;
```

Итог:

* эффект пересчитается, когда поменяется `a.v`;
* изменения `b.v` **сами по себе** эффект не триггерят;
* всё, что внутри `untracked`, не «приклеивает» новые сигналы к текущему эффекту.

Ментальная модель:

> `untracked` — это «сделать что-то с сигналами,
> но не превращать это в зависимость текущего эффекта».
