...
### 5. `untracked`: читать состояние, не подписываясь

Иногда нужно **один раз заглянуть в сигнал**,
но не хочется, чтобы эффект зависел от него.

Например, логируем старое значение перед обновлением:

```ts
const count = signal(0);

effect(() => {
  console.log('Подписанный эффект:', count.v);
});

function increment() {
  // просто лог, который не должен перезапускать эффект
  untracked(() => {
    console.log('Перед increment, значение было:', count.v);
  });
...
* `effect` будет реагировать только на `count.v` внутри себя.
* Чтение `count.v` внутри `untracked` не создаёт зависимостей и не влияет на граф.

`untracked` — это способ сказать:
**«Я сейчас читаю ради любопытства, не подписывай меня на это»**.
